...
ENTRYPOINT : 컨테이너가 시작했을때 실행할 명령어 설정
...
RUN : 빌드시 (이미지 생성 과정) 컨테이너 내부에서 실행할 명령어 설정
#RUN 이미지 빌드하는 과정 실행
#ENTRYPOINT 이미지 생성 후 -> 컨테이너 실행할때 수행할 명령어
...
#내부 bash 접근 
#이미지 빌드과정에서 해당 리눅스 환경에 git 정상적으로 설치가 됐다?  확인
# git 버전 체크
git -v 
