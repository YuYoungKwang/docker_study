...
# busybox 이름의 컨테이너에게 sh(cmd커맨드창) 명령어를 수행하도록 실행하겠다
# -it 실시간 대화형 (현재 터미널창에 컨테이너 sh 이 연결된 상태로 사용)

# 리눅스 os 환경 sh (커맨드창) 접속 된 상태
# 리눅스 OS 사용 -> 리눅스 명령어 
ls          #현재 경로 파일목록 
hostname    #리눅스입장에서 본인 hostname
whoami      #접속중인 계정
pwd         #현재 위치한 경로 표시
cd          #경로(폴더) 이동
...
docker exec -it 6c4ba96fa182 bash
docker exec -it 6c4ba96fa182 sh
