...
# 컨테이너 내부에 저장되는 데이터 -> 컨테이너 외부에 데이터를 저장하기위한 개념 -> 도커 볼륨
...
# bccd064dbc27
# 내부 명령어 실행 -> 내부 접근
docker exec -it bccd064dbc27 bash
...
# 생성 확인 후, 기존 만들어진 컨테이너 삭제 -> 다시실행
docker rm -f bccd064dbc27

docker run -d -e MYSQL_ROOT_PASSWORD=pw12341234 -p 3306:3306 --name mysqldb mysql

# 다시 컨테이너 실행한 상태에서 접속해서, 아까 만들어든 testdb 가 유지/존재 확인
